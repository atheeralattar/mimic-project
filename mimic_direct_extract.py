...
def save_numerics(
    ...
    X.astype({k: int for k in ID_COLS})
...
def save_notes(notes, outPath=None, notes_h5_filename=None):
    ...
    nlp.add_pipe("component", before='parser')  # insert before the parser
...
def plot_variable_histograms(col_names, df):
    ...
    for c, vals in df.iteritems():
        ...
        if vals.quantile(.99)>0:

            plt.xlim(0, vals.quantile(0.99))
            
            plt.savefig(os.path.join(outPath, (str(c) + '_HIST_.png')), bbox_inches='tight')
            plt.close()
